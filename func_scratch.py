...
def gen_result(imgPath, dstPath, modelPath, batch_size, resolution):
    ...
    # model
    with slim.arg_scope(jointDis.jointDis_arg_scope(is_training=False)):
        output = jointDis.jointDis(frame_1, frame_2)
    ...
    with tf.Session() as sess:
        ...
        for idx in range(0, len_set, batch_size):
            ...
            '''
            spList = ['spatial', 'temporal']
            for sp_idx in range(0,2):
                out_cur = net_output[sp_idx]
                for k in range(0, out_cur.shape[0]):
                    tmp = out_cur[k, :, :, :]
                    tmp = cv2.resize(tmp, (sL[k][1], sL[k][0]))
                    #blured = ndimage.gaussian_filter(tmp, sigma=3)
                    blured = tmp
                    ...
                    (filepath, frame) = os.path.split(img_path)
                    nameidx = frame.rfind('_')
                    savepath = dstPath + spList[sp_idx] + '/' + frame[:nameidx] + '/'
                    if not os.path.isdir(savepath):
                        os.makedirs(savepath)
                    scipy.misc.imsave(savepath + frame, sal_map)
                    #print(savepath + frame)
...
if __name__ == '__main__':
    resList = {'64','96','128','256'}
    batch_size = 256
    teacher = 'DVA'

    imgPath = '/media/cvmedia/Data/fukui/DIS/AVS1K/testSet/Frame/'
    for resolution in resList:
        dstPath = 'results-new/jointDis/{}/{}/default/'.format(resolution, teacher)
        modelPath = 'models/jointDis/{}/{}/default/'.format(resolution, teacher)
